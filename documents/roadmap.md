...
10) Scale and Distance Measurement
10.1) A map may have an associated scale (`km_per_pixel`) that gives physical meaning to pixel distances.
10.2) The user can set the scale by entering a value directly or by calibrating with two reference points and a known distance.
10.3) When a scale is defined, the user can measure the straight-line distance between any two points on the map by clicking them.
10.4) The measured distance is displayed in appropriate units (km, or m if under 1 km).
10.5) Distance measurement is a read-only tool — it does not create or modify any data.
10.6) Maps without a defined scale do not offer the distance measurement tool.

11) Time (Base Support)
11.1) The system must be designed so the schema does not block future time event implementation.
11.2) The planned `TimeEvent` model will use `valid_from` and `valid_to` dates and reference entities via a join table.
11.3) The MVP does not require timeline implementation.

12) Constraints
12.1) The system must be single-user.
12.2) No authentication is required in the MVP.
12.3) No permission system is required.
12.4) The system must run entirely via docker compose.
